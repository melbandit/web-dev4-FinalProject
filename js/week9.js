...
TwitterApi.init();
var PinterestApi = (function(options){
    var shared = {},
        options = options || {};
    ...
    function searchPinterest( name ){
        //$results = $form.find('#pinterest ul');

        $('form[name=pinterest] button').click(function(event) {
            var $e = $(event.currentTarget),
                $form = $e.closest('form'),
                params = {},
                $results = $form.find('#pinterest ul'),
            ...
            var app_id = '4871638940192352435';
            var app_secret = '62027eface7857a149aa36b1f1cb0002149ca15d553871a717eaedab1ba39bde';
           // Authenticate via API Key
            var code= '23a16f8700ff3c78';
            var access_token = 'Ac_oT9Z48Za0g-A0CblLDvMEFsW5FI1soDUD0VNDm5SjviAv_QAAAAA';
            var endpoint = 'https://api.pinterest.com/v1/?access_token=' + access_token + '&fields=id,creator,note&limit=10';
            ...
            $.ajax({
                ...
                // for (var i = rsp.response.posts.photos.length - 1; i >= 0; i--) {
                //     var status = rsp.response.posts.photos[i];
                //     var imageUrl = '<img src="' + status.original_size + '"/>';
    ...
    return {
        init: init,
        searchPinterest: searchPinterest
    }
}());
