...
function trend_search($options) {
	global $connection;
	$args = http_build_query($options);
	$response = $connection->get(API_BASE_URL . "/trends/closest.json?" . $args);
	echo json_encode($response);
}
function user_place_search($options) {
	global $connection;
	$args = http_build_query($options);
	$response = $connection->get(API_BASE_URL . "/trends/place.json?" . $args);
	echo json_encode($response);
}
